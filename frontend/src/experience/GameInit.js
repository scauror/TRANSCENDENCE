...
import { KEYBOARD } from './KeyboardManager.js';
...
export class Game {
	constructor(numeroShapeRef, config) {
		...
		this.mapScene.load('/assets/map_scene/tronStadium.glb', (gltf) => {
		...
		const sphMaterial = new THREE.MeshPhongMaterial({
			map: new THREE.TextureLoader().load(utils.loadAsset('texture/cyberSphere.png')),
			shininess: 100,
